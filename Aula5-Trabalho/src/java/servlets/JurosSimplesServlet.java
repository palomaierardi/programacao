...
public class JurosSimplesServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Juros Simples -  JavaEE</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h3><a href= 'index.html '>Voltar</a></h3");
            out.println("<h1>CALCULO DOS JUROS:</h1>");
            try{
                double VP = Double.parseDouble(request.getParameter("VP"));
            out.println("<h2>VP = "+VP+"</h2>");
            ...
            out.println("<h2> N = "+n +" meses</h2>");
            double vf = VP * (1 + (j/100) * n);
             out.println("<h2> VF = "+vf +"</h2>");
            }catch(Exception ex) {
